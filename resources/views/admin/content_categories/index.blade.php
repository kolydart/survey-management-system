...
@section('title', trans('quickadmin.content-categories.title') . ' | ' . trans('quickadmin.qa_list'))

@section('content')
    <h3 class="page-title">@lang('quickadmin.content-categories.title')</h3>
    @can('content_category_create')
    <p>
        <a href="{{ route('admin.content_categories.create') }}" class="btn btn-success">@lang('quickadmin.qa_add_new')</a>
                        ...
                        @foreach ($content_categories as $content_category)
                            <tr data-entry-id="{{ $content_category->id }}">
                                ...
                                <td field-key='slug'>{{ $content_category->slug }}</td>
                                    ...
                                    <a href="{{ route('admin.content_categories.show',[$content_category->id]) }}" class="btn btn-xs btn-primary">@lang('quickadmin.qa_view')</a>
                                    @endcan
                                    @can('content_category_edit')
                                    <a href="{{ route('admin.content_categories.edit',[$content_category->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')</a>
                                    @endcan
                                    @can('content_category_delete')
<form action="{{ route('admin.content_categories.destroy', $content_category->id) }}" method="POST" style="display: inline-block;" onsubmit="return confirm('{{ trans('quickadmin.qa_are_you_sure') }}');">
                                        @csrf
                                        @method('DELETE')
