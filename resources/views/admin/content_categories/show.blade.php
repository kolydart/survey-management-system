...
@section('title', trans('quickadmin.content-categories.title') . ' | ' . trans('quickadmin.qa_view') . ' | ' . $content_category->id)
...
<form action="{{ route('admin.content_pages.destroy', $content_page->id) }}" method="POST" style="display: inline-block;" onsubmit="return confirm('{{ trans('quickadmin.qa_are_you_sure') }}');">
                                        @csrf
                                        @method('DELETE')
        ...
        @endif
            ...
            <a href="{{ route('admin.content_categories.index') }}" class="btn btn-default">@lang('quickadmin.qa_back_to_list')</a>
        </div>
    </div>
@stop
