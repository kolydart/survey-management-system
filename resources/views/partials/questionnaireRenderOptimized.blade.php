{{--
Optimized version of questionnaireRender.blade.php using services
injected models: 
$survey, 
(if Questionnaire@show): $questionnaire 
$statisticsService (optional)
$chartService (optional)
        ...
        <section 
            ...
            {{-- answer(s) --}}
                ...
                @elseif ($item->question->answerlist->type == 'radio' || $item->question->answerlist->type == 'checkbox')
                        ...
                        @if (\Route::currentRouteName() == 'admin.surveys.show')
                            @if ($item->label != 1)
                                ...
                                @if (\Request::query('rawdata')) 
                                    @include('partials.answerDataOptimized', ['item' => $item])
                                @else 
                                    @include('partials.answerChartOptimized', ['item' => $item])
                                @endif
                                {{-- report end--}}
                            @endif
                    ...
                    {{-- display any content value in admin.surveys.show using service --}}
                    @if (\Route::currentRouteName() == 'admin.surveys.show' && isset($statisticsService))
                        @php
                            $contentValues = $statisticsService->getResponseDataForStatistics($item->survey_id, $item->question_id);
                            $nonEmptyContent = array_filter($contentValues, function($value) { return !empty(trim($value)); });
                        @endphp
                        @if (!empty($nonEmptyContent))
                            <div class="col-md-6 col-lg-6 col-lg-offset-3">
                                <table class="table table-condensed table-hover table-bordered text-muted small">
                                    <tbody>
                                        @foreach ($nonEmptyContent as $row)
                ...
                {{-- single value by route begin --}}
                    ...
                    {{-- admin.surveys.show --}}
                    @else
                        {{-- display results (table, or csv) using service --}}
                        <div class="card col-md-6 col-lg-6 col-lg-offset-3">
                            @if (in_array($item->question->answerlist->type,["text"]) )
                                <table class="table table-condensed table-hover table-bordered">
                                    <tbody>
                                        @php
                                            $contentValues = isset($statisticsService) 
                                                ? $statisticsService->getResponseDataForStatistics($item->survey_id, $item->question_id)
                                    ...
                                    @php
                                        $contentValues = isset($statisticsService) 
                                            ...
                                            : App\Response::whereIn('questionnaire_id',$item->survey->questionnaires->pluck('id'))
                                                ->where('question_id',$item->question_id)->pluck('content')->toArray();
                                    @endphp
                                    {{implode(", ", $contentValues)}}
                                </div>
                            @endif
                        </div>
                        
                        {{-- statistics using service --}}
                        @if (isset($statisticsService))
                            @php
                                $responseData = $statisticsService->getResponseDataForStatistics($item->survey_id, $item->question_id);
                                $supportsStats = $statisticsService->supportsStatistics($item->question->answerlist->type);
                                $statistics = $supportsStats ? $statisticsService->calculateStatistics($responseData) : null;
                            @endphp
                            @if ($statistics && $supportsStats)
                                <div class="col-md-6 col-lg-6 col-lg-offset-3">
                                    {{ $statisticsService->getStatisticsText($statistics, $item->question->answerlist->type) }}
                                </div>
                            @endif
                        @else
                            {{-- Fallback to original logic --}}
                            @php 
                                $array = App\Response::whereIn('questionnaire_id',$item->survey->questionnaires->pluck('id'))
                                    ->where('question_id',$item->question_id)->pluck('content')->toArray(); 
                            @endphp
